...
- verify immutable AnalysisRun scope persistence, indexed run-scoped item/manifest/task/model-run/proposal queries without corpus scans, selected execution reconciliation, restart-stable coverage, and no run-local orphaned model runs with `uv run pytest packages/application/tests/test_grounded_candidates.py packages/adapters/tests/test_sqlite_ledger_migrations.py packages/adapters/tests/test_docling_pdf_r1a.py`
