...
- verify every encrypted credential path, controlled corruption, third-party corrupt/fuzzed source, parser subprocess exit, forced termination, timeout, and OCR crash crosses public PDF ingestion with credential-safe typed accounting, retained raw bytes, no partial complete representation, archived versioned repair, and restart-safe retry using `uv run pytest packages/adapters/tests/test_pdf_failure_matrix.py`
...
- verify User Ingestion Run creation, guarded terminal history, configured first-use storage, deposited PDF/Markdown/text reuse, safe typed failures, and locked-PDF user CLI closeout with `uv run pytest packages/domain/tests/test_ingestion_runs.py packages/application/tests/test_ingestion_runs.py packages/adapters/tests/test_sqlite_ingestion_runs.py packages/pipelines/tests/test_user_ingestion_run.py packages/pipelines/tests/test_source_add_file.py && uv run python scripts/verify_cir1_canonical.py`
