...
- verify the exact TDD 4 PDF gold-matrix partition crosses capture, attempt-scoped durable preflight and per-page status, selected transformation, canonical representation, mandatory acceptable/degraded/blocked/failed quality routing, context planning or explicit policy-visible review/block, an archived-page evidence overlay rendered without Docling for born-digital/OCR/rotated/table evidence, run-scoped coverage, SQLite restart, and deterministic rerun with `KOTEKOMI_QPDF_EXECUTABLE=/absolute/path/to/qpdf-11.9.0 uv run pytest packages/adapters/tests/test_pdf_integrated_gold_matrix.py`
