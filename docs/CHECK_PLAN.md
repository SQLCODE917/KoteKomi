...
- verify frozen AnalysisPlan persistence, unit/manifest/model-run/proposal reconciliation, restart-stable incomplete coverage, and no orphaned model runs with `uv run pytest packages/application/tests/test_grounded_candidates.py packages/adapters/tests/test_docling_pdf_r1a.py`
