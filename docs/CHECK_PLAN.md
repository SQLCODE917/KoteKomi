...
- verify immutable AnalysisRun scope persistence, selected unit/manifest/task/model-run/proposal reconciliation, restart-stable coverage, and no run-local orphaned model runs with `uv run pytest packages/application/tests/test_grounded_candidates.py packages/adapters/tests/test_docling_pdf_r1a.py`
