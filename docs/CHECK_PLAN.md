...
- verify the exact TDD 4 PDF gold-matrix partition crosses capture, preflight, per-page status, selected transformation, canonical representation, quality policy, context planning or explicit block, evidence overlay, run-scoped coverage, SQLite restart, and deterministic rerun with `uv run pytest packages/adapters/tests/test_pdf_integrated_gold_matrix.py`
