...
- verify bounded staged extraction admits only the manifest-pinned model profile and tokenizer, archives every model response, rejects out-of-manifest local references before invocation, preserves later SQLite retry runs and review decisions, atomically commits successful runs with candidate batches, and maps valid PDF candidates to canonical pending proposals with `uv run pytest packages/application/tests/test_grounded_candidates.py packages/adapters/tests/test_docling_pdf_r1a.py`
