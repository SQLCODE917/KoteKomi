...
| HP-2 Document Reference Resolution | A reviewer can inspect explicit aliases and unresolved document references. | HP-1 preserves ontology-neutral MentionCandidates and interpretations. | The Pipeline emits ReferenceDecisions without inventing antecedents. |
| HP-3 Entity Identity Grounding | A reviewer can inspect known identity candidates only for specific mentions. | HP-2 identifies specific mentions and document-local aliases. | ReFinED emits ranked EntityLinkCandidates or NIL after contextual interpretation. |
| HP-4 Event Frame Drafts | A reviewer can inspect one source-grounded event with all participant roles. | HP-3 supplies interpreted and identity-grounded mentions. | Qwen2.5 emits bounded EventFrameDrafts with task-local references. |
