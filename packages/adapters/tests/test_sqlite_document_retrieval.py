...
from kotekomi_application.document_retrieval import ProjectionBuildInput
...
def test_sqlite_document_retrieval_is_exact_lexical_and_rebuildable(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert [
            candidate.retrieval_unit_id for candidate in exact
        ] == [build.units[0].retrieval_unit_id]
        assert [
            candidate.retrieval_unit_id for candidate in lexical
        ] == [build.units[0].retrieval_unit_id]
