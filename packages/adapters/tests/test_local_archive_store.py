...
def test_put_reuse_restart_and_corruption_rejection_for_hybrid_proposal_plan(
    tmp_path: Path,
) -> None:
    store = LocalArchiveStore(tmp_path)
    store.initialize()
    ...
    created = store.put_hybrid_proposal_plan(plan, payload, digest)
    reused = store.put_hybrid_proposal_plan(plan, payload, digest)
    reopened = LocalArchiveStore(tmp_path)

    assert created.disposition is ArchivePutDisposition.CREATED
    assert reused.disposition is ArchivePutDisposition.REUSED
    assert reopened.read_hybrid_proposal_plan(plan.id) == payload

    stored = tmp_path / "extraction" / "proposal-plans" / f"{plan.id}.json"
    stored.write_bytes(b"different bytes")
    with pytest.raises(ValueError, match="conflicts with its immutable identity"):
