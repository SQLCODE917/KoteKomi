...
def build_evidence_graph_projection(
    ...
    *,
    ...
    del command
    try:
        edges, contributions, snapshot = build_evidence_graph_state(ledger_repository)
        manifest = _manifest(snapshot, edges, contributions)
        published, reused = projection.publish_evidence_graph(
            EvidenceGraphProjectionBuildInput(manifest, edges, contributions)
        )
...
def explain_evidence_graph_relationship(
    ...
    *,
    ...
    try:
        ...
        _, _, snapshot = build_evidence_graph_state(ledger_repository)
...
def build_evidence_graph_state(
...
) -> tuple[tuple[EvidenceGraphEdge, ...], tuple[EvidenceGraphContribution, ...], str]:
    ...
    return tuple(edges), tuple(contributions), snapshot
