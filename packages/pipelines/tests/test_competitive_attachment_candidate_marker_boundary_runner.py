...
def _runner() -> Any:
    ...
    return importlib.import_module("run_competitive_attachment_candidate_marker_boundary")


def test_generation_contract_preserves_whole_arm_runtime_settings() -> None:
    runner = _runner()
    config = cast(
        PipelineConfig,
...
def test_complete_status_exposes_review_package_without_canonical_changes(
    ...
    report = cast(
        AttachmentCandidateMarkerReport,
        SimpleNamespace(
            outcome=AttachmentCandidateMarkerOutcome.SUPPORTED,
            model_execution_count=40,
        ),
    )
