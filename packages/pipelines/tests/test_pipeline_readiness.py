...
def pipeline_status_json(
    ...
    assert main(pipeline_status_json_args(ledger_path)) == 0
...
def pipeline_next_json(
    ...
    assert main(pipeline_next_json_args(ledger_path)) == 0
    payload = json.loads(capsys.readouterr().out)
    assert isinstance(payload, dict)
    return cast(dict[str, Any], payload)
...
def pipeline_status_json_args(ledger_path: Path) -> list[str]:
    return [
        "pipeline",
        "status",
        "--format",
...
def pipeline_next_json_args(ledger_path: Path) -> list[str]:
    return [
        "pipeline",
        "next",
        "--format",
