...
def _runner() -> Any:
    ...
    return importlib.import_module("run_competitive_attachment_partwise_residual_ownership")


def test_generation_contract_declares_runtime_penalty_and_two_token_budget() -> None:
    runner = _runner()
    config = cast(
        PipelineConfig,
...
def test_complete_status_exposes_review_package_without_canonical_changes(
    ...
    report = cast(
        AttachmentPartwiseReport,
        SimpleNamespace(
            outcome=AttachmentPartwiseOutcome.SUPPORTED,
            model_execution_count=54,
        ),
    )

    status = runner._status(
        tmp_path,
        AttachmentPartwisePackageStatus.COMPLETE,
        report=report,
    )
