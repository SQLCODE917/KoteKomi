...
def test_hybrid_event_semantics_profile_is_bounded_canonical_and_source_agnostic() -> None:
    ...
    assert HYBRID_EVENT_SEMANTICS_V4.upper_roles == tuple(
        sorted(UpperRole, key=lambda item: item.value)
    )
    assert tuple(item.id for item in HYBRID_EVENT_SEMANTICS_V4.frames) == (
        "agreement",
        "authorization",
        "causation",
        "change_in_intensity",
    ...
    assert {item.id for item in frames["criticism"].roles} >= {
        "criticism.assessment",
        "criticism.critic",
        "criticism.target",
    }
    assert {item.id for item in frames["publication"].roles} >= {
        "publication.outlet",
        "publication.published_work",
...
def test_hybrid_event_semantics_roles_are_frame_scoped_and_not_metadata() -> None:
    roles = {role.id: role for frame in HYBRID_EVENT_SEMANTICS_V4.frames for role in frame.roles}
...
def test_temporal_relation_is_a_bounded_governed_vocabulary() -> None:
    assert tuple(TemporalRelation) == (
